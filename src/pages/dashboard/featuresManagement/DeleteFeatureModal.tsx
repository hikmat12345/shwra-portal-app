...
import { deletePromotion } from 'redux/slices/promotions';

type DeleteFeatureModalModalProps = {
  open: boolean;
  setOpen: (open: boolean) => void;
  data: { name: string; id: string | number };
};
export default function DeleteFeatureModalModal({ open, setOpen, data }: DeleteFeatureModalModalProps) {
      ...
      <DialogContent>
        ...
        <Box>
          <Typography variant="h5" sx={{ textAlign: 'center' }}>
            هل انت متاكد من حذف كود الخصم {data?.name}
          </Typography>{' '}
        </Box>
        <Box sx={{ p: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Button
            onClick={() => {
              dispatch(deletePromotion(data?.id));
              setOpen(false);
            }}
            variant="contained"
            color="success"
