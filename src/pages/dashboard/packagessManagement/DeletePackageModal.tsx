...
import { deletePackage } from 'redux/slices/packages';
...
export default function DeletePackageModal({ open, setOpen, data }: DeletePackageModalProps) {
      ...
      <DialogContent>
        ...
        <Box>
          <Typography variant="h5" sx={{ textAlign: 'center' }}>
            هل انت متاكد من حذف باقة {data?.name}
          </Typography>{' '}
        </Box>
        <Box sx={{ p: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Button
            onClick={() => {
              dispatch(deletePackage(data?.id));
              setOpen(false);
            }}
            variant="contained"
            color="success"
